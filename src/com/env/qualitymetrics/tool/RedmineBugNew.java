...
//缺陷新增率
public class RedmineBugNew {
	private static final Logger log = LoggerFactory.getLogger(RedmineBugNew.class);
	 ...
	 * @return
	 */
	public float getBugNewRate(String projectName,String versionNames,String startDate,String endDate){
		float rate=-1;
		...
		String sql_new="select count(0) from issues "
			...
			+"and issues.tracker_id=1 and issues.status_id!=6 and issues.created_on between ? and ?";
		...
		log.info("There are "+newCount+" bugs in "+projectName+" version"+version);
		...
		if((featureCount==0)&&(fixCount==0)){
			log.error("There are no feature and bug in "+projectName+" version"+version);
			return rate;
		}
		log.info("There are "+featureCount+" features and "+fixCount+" bugs fixed in "+projectName+" version"+version);
