...
//缺陷遗漏率
public class RedmineBugOmissionForCategory{
	private static final Logger log = LoggerFactory.getLogger(RedmineBugOmissionForCategory.class);
	 ...
	 * @return
	 */
	public float getBugOmissionRate(String projectName,String versionNames,String categoryNames,String supportName,String startDate,String endDate){
		float rate=-1;
		int flag=this.setMap(projectName, versionNames, categoryNames,supportName,startDate, endDate);
		if(flag==-1) {
		...
		log.info("b grade is "+b+", total grade is "+total+" in "+projectName+" version<"+version+"> category <"+category+"> between "+startDate+" and "+endDate);
		if((total==0)&&(b==0)){
			log.error("There are no bug in "+projectName+" version<"+version+"> category <"+category+"> between "+startDate+" and "+endDate);
			return rate;
		}
		rate=b/(total+b);
	...
	public int setMap(String projectName,String versionNames,String categoryNames,String supportName,String startDate,String endDate){
		int flag=-1;
		...
		for(int i=0;i<nameArrays.length;i++){
			String field2="issues.category_id";
